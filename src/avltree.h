...
typename tree<type>::node* tree<type>::rotateLeft(node *n) {
  ...
  rightChild->height = max(rightChild->getHeight(rightChild->left),
      rightChild->getHeight(rightChild->right)) + 1;
...
typename tree<type>::node* tree<type>::rotateRight(node *n) {
  ...
  n->height = max(n->getHeight(n->left), n->getHeight(n->right)) + 1;
  leftChild->height = max(leftChild->getHeight(leftChild->left),
      leftChild->getHeight(leftChild->right)) + 1;
...
template<class type>
typename tree<type>::node* tree<type>::insert(type value, node *current) {
  if (current == NULL)
  ...
  current->height = max(current->getHeight(current->left),
      current->getHeight(current->right)) + 1;
...
template<class type>
typename tree<type>::node* tree<type>::erase(type value, node *current) {
  if (current == NULL)
    return NULL;
  ...
  current->height = max(current->getHeight(current->left),
      current->getHeight(current->right)) + 1;
  int balance = current->getBalance();
  int leftBalance = (balance > 0) ? current->left->getBalance() : 0;
  int rightBalance = (balance < 0) ? current->right->getBalance() : 0;
